import {P_VNRyCANCELAR_CM,EJECUTAR_SENTENCIA_BD_TABLET, getusuario,Execute,REGISTRA_T_LLAMADAS,REGISTRA_CM_AUDITORIA,P_envio_msj,dragElement,destinatario,MensBepeer,Prefijo,flag_Beeper,Gs_CodDr,Executeinsert,CREABEEPER_ATE} from './module.js'; 
 ...
 (async function(){ 
  ...
  .then(function(data) {
    ...
    if (data.length >0){
                  ...
                  if (Adata_atencion[0].enviar_correo == true ){
                      correo_proveedor = Adata_atencion[0].correo;
                  }else{
                      document.getElementById('Cmd_correo_prov').style.visibility = 'hidden';
                  }

                  if ( document.getElementById('Cmd_correo_prov').style.visibility == '' || document.getElementById('Cmd_correo_prov').style.visibility == 'visible' ){
                      
                      
                          if ( Adata_atencion[0].flagmone == "D" ){
                              //$ 20.00, TC: S/. 2.80
                              str_ded = "$ " + (Math.round((Adata_atencion[0].tar_ate / Adata_atencion[0].cambio + Number.EPSILON) * 100)/100).toFixed(2 )  + ", TC: S/. " +  Adata_atencion[0].cambio.toFixed(2 );
                          }else{
                              //S/. 34.00
                              str_ded = "S/. " + Adata_atencion[0].tar_ate.toFixed(2 );
                          }
                          ...
                          switch ( Adata_atencion[0].for_ate){
                              case "E":
                                  ...
                                  break;
                              case "T":
                                  str_forma_pago = "TARJETA";
                                  break;
                              case "C":
                                  str_forma_pago = "CREDITO";
                                  break;
                              case "F":
                                  str_forma_pago = "TRASFERENCIA";
                                  break;
                              default:
                                  str_forma_pago = "EFECTIVO";
                                  break;
                          ...
                          str_body = "PACIENTE   :" + Adata_atencion[0].nom_pac + '\n' 
                          + "EDAD             :" + Adata_atencion[0].edad_ate + '\n' 
                          + "DM               :" + Adata_atencion[0].cod_aso.padStart(8,'0')  + '\n' 
                          + "SÍNTOMAS         :" + Adata_atencion[0].sin_ate + '\n' 
                          + "DIRECCIÓN        :" + Adata_atencion[0].Direccion + " " + Adata_atencion[0].nro_dir_lote + " " + Adata_atencion[0].dir_dpto_interior + ", " + Adata_atencion[0].dir_urbanizacion + '\n' 
                          + "REFERENCIA       :" + Adata_atencion[0].ref_dir.trim() + '\n' 
                          + "DISTRITO         :" + Adata_atencion[0].des_dis.trim() + '\n' 
                          + "COD.AUTORIZACION :" + (Adata_atencion[0].cod_aut_prestacion == null? "": Adata_atencion[0].cod_aut_prestacion.trim()) + '\n' 
                          + "FIJO/MOVIL       :" + Adata_atencion[0].tlf_dir + "//" + (Adata_atencion[0].cel_pac==null ? "": Adata_atencion[0].cel_pac) + '\n' 
                          + "DEDUCIBLE        :" + str_ded + '\n' 
                          + "FORMA PAGO       :" + str_forma_pago + '\n' 
                          + "DENOMIN.         :" + (Adata_atencion[0].cm_denominacion==null?"EXACTO":  Adata_atencion[0].cm_denominacion.toUpperCase()) + '\n' 
                          + "COASEGURO        :" + Adata_atencion[0].coaseguro.toFixed(2 ) + " %" + '\n' 
                          + "CLIENTE          :" + Adata_atencion[0].nom_gru.trim() + '\n' 
                          + "RANGO            :" + Adata_atencion[0].fec_ate.split('-').reverse().join('-') + Adata_atencion[0].hor_ate  + '\n' + '\n';

                          
                          document.getElementById('Txt_body').value = str_body;
                          //Me.Height = 7695
                  ...
                  if (Adata_atencion[0].f_prog == "Prg" ){
                    document.getElementById('Lbl_mensa_prog').style.visibility = 'visible';
                    document.getElementById('Lbl_mensa_prog').innerHTML = "Hora programada: " + Adata_atencion[0].hor_ate + " Confirmar a Conductor";
                  }

                  if (Adata_atencion[0].flg_reprogramada == true ){
                    document.getElementById('Lbl_mensa_prog').style.visibility = 'visible';
                    document.getElementById('Lbl_mensa_prog').innerHTML  = "REPROGRAMADA: " + Adata_atencion[0].hor_ate + " Confirmar a Conductor";
                  }
...
window.Command1_Click = async function(){
  ...
  document.body.style.cursor = 'progress';
    if (Adata_atencion[0].cm_estado == "R5" ){
        await Execute ("UPDATE t_tmpllamadas SET cm_estado = 'R6', cm_orden = 7 WHERE cod_ate = " + document.getElementById('Txt_CodAte').value);
        await  REGISTRA_CM_AUDITORIA(document.getElementById('Txt_CodAte').value, l_estado_audi + "->R6", "MEDICO: " + document.getElementById('Txt_medico').value + ", CONDUCTOR: " + document.getElementById('Txt_conductor').value, "CONFIRMA RECEPCION DE MENSAJE");
    }else{
        await Execute ("UPDATE t_tmpllamadas SET cm_estado = '6', cm_orden = 7 WHERE cod_ate = " + document.getElementById('Txt_CodAte').value);
        await REGISTRA_CM_AUDITORIA(document.getElementById('Txt_CodAte').value, l_estado_audi + "->6", "MEDICO: " + document.getElementById('Txt_medico').value + ", CONDUCTOR: " + document.getElementById('Txt_conductor').value, "CONFIRMA RECEPCION DE MENSAJE");
    }
    document.body.style.cursor = 'default';

    alert("Se confirma la recepción de mensaje");
    appMainWindow.document.getElementById('Cbo_opcion').value = 14;
    appMainWindow.document.getElementById('Txt_busqueda').style.display='inline-block';
