                 ...
                 if(data.length>0){
                  ...
                  /*     If Cbo_pac_VIP.Text = "MINT" Then
                      ...
                      case "Prg":
                          ...
                          document.getElementById('DTPicker1').value = Adata_atencion[0].fec_ate.trim();
...
if (document.getElementById('CmbHora').style.visibility == 'visible' ){
    if (document.getElementById('CmbHora').value == ""){
        ...
        if (new Date().toLocaleDateString(undefined,{year:'numeric',month:'2-digit',day:'2-digit'}).replace(/\//g, '-').split('-').reverse().join('-') == document.getElementById('DTPicker1').value && new Date().toTimeString().slice (0,5) > document.getElementById('CmbHora').value ) {
...
if (Adata_atencion[0].f_prog == "Prg" && Adata_atencion[0].clasificacion_pac == 2 ){
      if (new Date().toLocaleDateString(undefined,{year:'numeric',month:'2-digit',day:'2-digit'}).replace(/\//g, '-').split('-').reverse().join('-') <   (Adata_atencion[0].feclla_ate).addDays(30).replace(/\//g, '-').split('-').reverse().join('-')) {
...
await fetch('/modulo/Abre_Detalle/', {
  ...
  switch (document.getElementById('Cbo_tipo_prog').selectedIndex){
        case 0:
            ...
            Adata_atencionu[0].fecasgdr_ate = new Date().toLocaleString(undefined, {year: 'numeric', month: '2-digit',  day: '2-digit' });
        ...
        case 2:
            ...
            Adata_atencionu[0].cm_tiempo = null;
            Adata_atencionu[0].horasgdr_ate = new Date().toLocaleTimeString();
            Adata_atencionu[0].fecasgdr_ate =  new Date().toLocaleString(undefined, { 
              year: 'numeric',
              month: '2-digit',
              day: '2-digit' });
              await fetch('/modulo/get_session/', {
...
function nomenoractual(e){
  ...
  var valida = new Date( e.target.value.replace(/-/g, ',') ).toLocaleDateString(undefined,{year:'numeric',month:'2-digit',day:'2-digit'}).replace(/\//g, '-').split('-').reverse().join('-');
  console.log(valida);
  // Days in JS range from 0-6 where 0 is Sunday and 6 is Saturday

  if( valida>=new Date().toLocaleDateString(undefined,{year:'numeric',month:'2-digit',day:'2-digit'}).replace(/\//g, '-').split('-').reverse().join('-')){
