...
<button type="button" @click="abrirmodalsubcategoria($event)" id="agregar" class="btn btn-success" >AGREGAR</button>&nbsp;&nbsp;&nbsp;
<button type="button" @click="abrirmodalsubcategoria($event)" id="editar" class="btn btn-warning" >EDITAR</button>&nbsp;&nbsp;&nbsp;
...
<table  id="tabla1" >
      ...
      <td  v-for="(id,name,index) in medicamento">  <span v-show="index!=4">{{id}}</span></td>               
      </tr>
      </tbody>
      <tfoot>
      <!-- Paginate -->
      ...
      <modalsubcategoria @cerrarmodalsubcategoria="oncerrarmodalsubcategoria" v-if="mostrarmodalsubcategoria" :medicamento="selectedMed" :tdinvoke="tdinvoke" ></modalsubcategoria>
...
var app = {
    ...
    async created() {
      ...
      await axios.get('/delivery/get_ecommerce_subcategoria_fields',{  responseType:'arraybuffer'  }).then(response => {
      ...
      await axios.get('/delivery/get_ecommerce_subcategoria',{  responseType:'arraybuffer'  }).then(response => {
...
App.component('modalsubcategoria',{
   template: 
   `<div class="ed-modal-container">
      <div class="ed-modal-content">
        ...
        <div style="display:grid;grid-template-columns: 1fr 1fr;">
                        ...
                        <div><label>Categoria</label></div><div><select id="categoria" :value="medicamento.id_categoria"  ></select></div>
                        <div><label for="activo">Activo</label></div> 
                        <div><input id="activo" type="checkbox" :checked="medicamento.activo=='ACTIVO'"></div> 
                <div style="display: flex;justify-content: center;">
                  <div>
                  <button type="submit" @click="guardarsubcategoria()" class="btn btn-success" >GUARDAR</button>&nbsp;&nbsp;&nbsp;
   ...
   methods:{
       ...
       seleccionarsubcategoria(){
        this.tdinvocado.innerHTML =  document.getElementById('subcategoria').value;
       ...
       async guardarsubcategoria(){
        ...
        if ( this.tdinvocado.id == 'editar' ){
          await fetch('/delivery/get_ecommerce_update_subcategoria/', {
          method: 'POST',
          headers: {
          ...
          await fetch('/delivery/get_ecommerce_insert_subcategoria/', {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
   ...
   async mounted(){
     if ( this.tdinvocado.id == 'editar' ){
         ...
         await fetch('/delivery/get_ecommerce_id_last_subcategoria/', {
          method: 'POST',
          headers: {
            'Accept': 'application/json',
            'Content-Type': 'application/json'
    ...
    if ( this.tdinvocado.id == 'editar' ){ 
       document.getElementById('categoria').value = this.medicamento.id_categoria;
    }else if(this.tdinvocado.id == 'agregar' ){
