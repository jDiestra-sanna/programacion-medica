...
class Ion_auth
{
	...
	public function logged_in()
	{
		...
		if (!$recheck){
			$recheck = $this->ion_auth_model->login_remembered_dr();
		}
	 ...
	 * @author Ben Edmunds
	 */
	public function is_admin($admin_group ='PROGRAMACION MEDICA - SUPERVISOR',$id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_admin');

		$admin_group || $admin_group = $this->config->item('admin_group', 'ion_auth');

		return $this->ion_auth_model->in_group($admin_group, $id);
	}
    public function is_admincodigo($codigo =0,$id = FALSE)
	{
		$this->ion_auth_model->trigger_events('is_admin');

		return $this->ion_auth_model->in_groupcodigo($codigo, $id);
	}
	public function get_menus( $admin_group ='PROGRAMACION MEDICA - SUPERVISOR')
	{
		$this->ion_auth_model->trigger_events('is_admin');

		$admin_group || $admin_group = $this->config->item('admin_group', 'ion_auth');

		return $this->ion_auth_model->get_menus($admin_group);
	}
	/**
