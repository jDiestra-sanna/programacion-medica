 ...
 * @property Ion_autffh $ion_auth The Ion_auth library
 */
...
/* 		if (strpos($hash_password_db, '$') === 0)
	...
	public function clear_forgotten_password_code_dr($identity) {
		...
		$this->db->update('m_doctores', $data, ['login'=> $identity]);
	...
	public function clear_remember_code_dr($identity) {
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select('m_usuarios.'.$this->identity_column . ', email, cod_usu, pas_usu, m_usuarios.activo, last_login,cod_prov_medico,cod_prov_motorizado,trim(m_proveedor_motorizado.descripcion) as nom_prov_motorizado ')
						  ->where("trim(m_usuarios.nom_usu)",strtoupper ($identity))
						  ->where("trim(pas_usu)", $password)
						  ->join('m_proveedor_motorizado', 'trim(m_usuarios.nom_usu) = m_proveedor_motorizado.nom_usu', 'left')
						  ->join('m_proveedor_medico', 'trim(m_usuarios.nom_usu) = m_proveedor_medico.nom_usu', 'left')
						  ->limit(1)
						  ->order_by('cod_usu', 'desc')
						  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			/* if ($this->verify_password($password, trim($user->pas_usu), $identity))
			{*/
				if (!$user->activo)
				{
					$this->trigger_events('post_login_unsuccessful');
				...
				$this->update_last_login($user->cod_usu);
		...
		//	}
				...
				$query2 = $this->db->select('nom_doc, medico_email, login,cod_doc,  activi, last_login')
				->where("trim(login)",strtoupper($identity))
				->where('num_doc_id', $password)
				->limit(1)
				->order_by('cod_doc', 'desc')
				->get('m_doctores');
					if ($query2->num_rows() === 1)
					{
						...
						$this->set_sessiondr($userdr);

						$this->update_last_login_dr($userdr->cod_doc);

						$this->clear_login_attempts($identity);
						$this->clear_forgotten_password_code_dr($identity);

						if ($this->config->item('remember_users', 'ion_auth'))
						{
							if ($remember)
							{
								$this->remember_dr($identity);
							}
							else
							{
								$this->clear_remember_code_dr($identity);
							}
						}
		...
		if ($recheck !== 0)
		{
			...
			if ($last_login + $recheck < time())
			{
				...
				else
				{
					...
					$this->session->unset_userdata([$identity, 'nom_usu' , 'user_id' ]);
	...
	public function users($groups = NULL)
	{
		...
		else
		{
			...
			$this->db->select([
			    $this->tables['users'].'.*',
			    $this->tables['users'].'.nom_usu as id',
			    $this->tables['users'].'.nom_usu as user_id'
			]);
		}
	 ...
	 * @author Ben Edmunds
	 */
	public function get_users_groups_codigo($id = FALSE)
	{
		$this->trigger_events('get_users_group');

		// if no id was passed use the current users id
		$id || $id = trim($this->session->userdata('nom_usu'));

		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as cod_permiso, '.$this->tables['groups'].'.des_permiso, '.$this->tables['groups'].'.activo')
						->where('trim('.$this->tables['users_groups'].'.'.$this->join['users'].')', trim($id))
						//->where_in($this->tables['groups'].'.'.$this->join['groups'],[94,137,138,139,140,141,142,143,302,303,304,200,201,400,501,900,601,700,110] )
						->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.cod_permiso')
	...
	public function get_users_groups($id = FALSE)
	{
		...
		return $this->db->select($this->tables['users_groups'].'.'.$this->join['groups'].' as cod_permiso, '.$this->tables['groups'].'.des_permiso, '.$this->tables['groups'].'.activo')
						->where('trim('.$this->tables['users_groups'].'.'.$this->join['users'].')', trim($id))
						->where_in($this->tables['groups'].'.'.$this->join['groups'],[94,137,138,139,140,141,142,143,144,302,303,304,305,200,201,400,401,501,900,601,700,701,110] )
						->join($this->tables['groups'], $this->tables['users_groups'].'.'.$this->join['groups'].'='.$this->tables['groups'].'.cod_permiso')
		                ->get($this->tables['users_groups']);
	}
	...
	public function in_group($check_group, $id = FALSE, $check_all = FALSE)
	{
		...
		else
		{
			...
			{
				$groups_array[$group->cod_permiso] =trim( $group->des_permiso);
			}
	...
	public function in_groupcodigo($check_group, $id = FALSE, $check_all = FALSE)
	{
		$this->trigger_events('in_group');
		...
		{
			$users_groups = $this->get_users_groups_codigo($id)->result();
			$groups_array = [];
			foreach ($users_groups as $group)
			{
				$groups_array[$group->cod_permiso] =trim( $group->cod_permiso);
			}
			$this->_cache_user_in_group[$id] = $groups_array;
		}
		foreach ($check_group as $key => $value)
	...
	public function get_menus($check_group, $id = FALSE, $check_all = FALSE)
	{
		//$this->trigger_events('in_group');
		...
		$query = $this->db->query("select  distinct des_permiso from m_permiso join m_permisoxusuario on m_permisoxusuario.cod_permiso = m_permiso.cod_permiso where 
		nom_usu='".trim($this->session->userdata('nom_usu'))."' and idparent is not null ");
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], ['last_login' => time()], ['cod_usu' => $id]);

		return $this->db->affected_rows() == 1;
	}
	public function update_last_login_dr($id)
	{
		$this->trigger_events('update_last_login');
		...
		$this->db->update('m_doctores', ['last_login' => time()], ['cod_doc' => $id]);
	...
	public function set_session($user)
	{
		...
		$session_data = [
		    ...
		    'email'                    => $user->email,
		    'user_id'                  => trim($user->cod_usu), //everyone likes to overwrite id so we'll use user_id
		    'old_last_login'           => $user->last_login,
		    'last_check'               => time(),
			'ion_auth_session_hash'    => $this->config->item('session_hash', 'ion_auth'),
			'proveedor'                => $user->cod_prov_motorizado,
			'cod_prov_medico'          => $user->cod_prov_medico,
			'nom_proveedor'            => $user->nom_prov_motorizado,
		];
	...
	public function set_sessiondr($user)
	{
		$this->trigger_events('pre_set_session');

		$session_data = [
		    'identity'                 => $user->login,
//			'nom_usu'                 => $user->nom_usu, //pedro03032022
			$this->identity_column     => $user->login,
			'des_usu'				   => $user->nom_doc,
			'email'                    => $user->medico_email,
			$this->identity_column     => $user->login,
		    'user_id'                  => trim($user->cod_doc), //everyone likes to overwrite id so we'll use user_id
			'old_last_login'           => $user->last_login,
			//'user_id'                  => trim($user->nom_doc), //everyone likes to overwrite id so we'll use user_id
		    'last_check'               => time(),
		    'ion_auth_session_hash'    => $this->config->item('session_hash', 'ion_auth'),
		];
	...
	public function remember_dr($identity)
	{
		$this->trigger_events('pre_remember_user');
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select('m_usuarios.'.$this->identity_column . ', cod_usu, email, remember_code, last_login,cod_prov_medico,cod_prov_motorizado,trim(m_proveedor_motorizado.descripcion) as nom_prov_motorizado')
						  ->where('remember_selector', $token->selector)
						  ->where('m_usuarios.activo', true)
						  ->join('m_proveedor_motorizado', 'trim(m_usuarios.nom_usu) = m_proveedor_motorizado.nom_usu', 'left')
						  ->join('m_proveedor_medico', 'trim(m_usuarios.nom_usu) = m_proveedor_medico.nom_usu', 'left')
	...
	public function login_remembered_dr()
	{
		$this->trigger_events('pre_login_remembered_user');
		...
		// get the user with the selector
		$this->trigger_events('extra_where');
		$query = $this->db->select('m_doctores.login, m_doctores.cod_doc, m_doctores.medico_email, m_doctores.remember_code,m_doctores.nom_doc, m_doctores.last_login')
						  ->where('remember_selector', $token->selector)
						  ->where('activi', true)
						  //->join('m_usuarios', 'trim(m_doctores.doc_identidad) = trim(m_usuarios.num_doc_id)', 'left')
						  ->limit(1)
						  ->get('m_doctores');
		...
		if ($query->num_rows() === 1)
		{
			...
			//if ($this->verify_password($token->validator, $user->remember_code, $identity))
		//	{
				$this->update_last_login_dr($user->cod_doc);

				$this->set_sessiondr($user);
